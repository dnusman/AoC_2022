...
while q:
    time, pressure_released, own_route, elephant_route = q.popleft()
    
    # loop over unvisited valves
    unvisited_valves = [valve for valve in valves if valve not in own_route and valve not in elephant_route]
    if not unvisited_valves:
        pressures.append(pressure_released)
        continue
    ...
    for next_cave in unvisited_valves:
        ...
        if time + dtime > 26:
            ...
            continue
        dpressure_released = (26 - time - dtime) * flow_rates[next_cave]
